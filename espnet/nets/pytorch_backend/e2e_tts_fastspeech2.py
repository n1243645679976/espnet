...
class FeedForwardTransformer(TTSInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument('--energy-predictor-layers', default=2, type=int)
        group.add_argument('--energy-predictor-chans', default=384, type=int)
        group.add_argument('--energy-predictor-kernel-size', default=3, type=int)
        group.add_argument('--energy-predictor-dropout', default=0.5, type=float)
        group.add_argument('--energy-embed-kernel-size', default=9, type=int)
        group.add_argument('--energy-embed-dropout', default=0.5, type=float)
        group.add_argument('--stop-gradient-from-energy-predictor', default=False, type=bool)

        group.add_argument('--pitch-predictor-layers', default=2, type=int)
        group.add_argument('--pitch-predictor-chans', default=384, type=int)
        group.add_argument('--pitch-predictor-kernel-size', default=3, type=int)
        group.add_argument('--pitch-predictor-dropout', default=0.5, type=float)
        group.add_argument('--pitch-embed-kernel-size', default=9, type=int)
        group.add_argument('--pitch-embed-dropout', default=0.5, type=float)
        group.add_argument('--stop-gradient-from-pitch-predictor', default=False, type=bool)
    ...
    def __init__(self, idim, odim, args=None):
        ...
        self.stop_gradient_from_pitch_predictor = args.stop_gradient_from_pitch_predictor
        self.stop_gradient_from_energy_predictor = args.stop_gradient_from_energy_predictor
        ...
        self.pitch_predictor = VariancePredictor(
            idim=args.adim,
            n_layers=args.pitch_predictor_layers,
            n_chans=args.pitch_predictor_chans,
            kernel_size=args.pitch_predictor_kernel_size,
            dropout_rate=args.pitch_predictor_dropout,
        )
        # NOTE(kan-bayashi): We use continuous pitch + FastPitch style avg
        self.pitch_embed = torch.nn.Sequential(
            torch.nn.Conv1d(
                in_channels=1,
                out_channels=args.adim,
                kernel_size=args.pitch_embed_kernel_size,
                padding=(args.pitch_embed_kernel_size - 1) // 2,
            ),
            torch.nn.Dropout(args.pitch_embed_dropout),
        )

        # define energy predictor
        self.energy_predictor = VariancePredictor(
            idim=args.adim,
            n_layers=args.energy_predictor_layers,
            n_chans=args.energy_predictor_chans,
            kernel_size=args.energy_predictor_kernel_size,
            dropout_rate=args.energy_predictor_dropout,
        )
        ...
        self.energy_embed = torch.nn.Sequential(
            torch.nn.Conv1d(
                in_channels=1,
                out_channels=args.adim,
                kernel_size=args.energy_embed_kernel_size,
                padding=(args.energy_embed_kernel_size - 1) // 2,
            ),
            torch.nn.Dropout(args.energy_embed_dropout),
        )
    ...
    def _forward(
        ...
        if is_inference:
            ...
            minshape = min(hs.shape[1], p_embs.shape[1], e_embs.shape[1])-1
            hs = hs[:,minshape]
            p_embs = p_embs[:, minshape]
            e_embs = e_embs[:, minshape]
            hs = hs + e_embs + p_embs
    ...
    def forward(self, xs, ilens, ys, olens, pitch, energy, spembs=None, durations=None, dlens=None, extras=None, *args, **kwargs):
        """Calculate forward propagation.
        ...
        durations = durations[:, : max(dlens)]

        ilens = ilens
#        if durations.shape[1] > xs.shape[1]+2:
 #           xs = torch.cat([xs, torch.ones(xs.shape[0], durations.shape[1] - xs.shape[1]-2).cuda().long()*xs[0][-1]], dim=1)
 #       elif durations.shape[1] < xs.shape[1]+2:
 #           xs = xs[:, :durations.shape[1]+2]

        # forward propagation
        before_outs, after_outs, d_outs, p_outs, e_outs = self._forward(
            xs, ilens, ys, olens, durations, spembs=spembs, ds=extras, is_inference=False, ps=pitch, es=energy
        )
...
#           )
    ...
    def inference(self, x,sp=None, ds=None,ps=None,es=None,alpha=1.0,use_teacher_forcing=False, spemb=None):
